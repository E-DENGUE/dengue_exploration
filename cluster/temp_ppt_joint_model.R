...
joint_temp_ppt_model <- readRDS("Data/climate/climate_model.rds")
...
advance_missing_func_temp <- function(N_advance){
  ...
  joint_temp_ppt_model_for_model <- joint_temp_ppt_model_2 %>% 
    ...
    mutate( m_DHF_cases=if_else(is.na(m_DHF_cases),0, m_DHF_cases), ##ALERT...assuming nov, dec missing=0
            ...
            rh = if_else(index==max(index,na.rm=T), 0, rh),
...
mod.func <- function(N_advance) {
  ...
  x_model <- cbind(x_model_1, ds$rh, ds$lag_1_rh, ds$lag_2_rh, ds$lag_3_rh, 
                   ds$lag_4_rh, ds$lag_5_rh, ds$lag_6_rh, ds$tp, ds$lag_1_tp, 
                   ds$lag_2_tp, ds$lag_3_tp, ds$lag_4_tp, ds$lag_5_tp, ds$lag_6_tp)
  ...
  # save to a file
  saveRDS(post_fit_joint, paste0("post_fit_joint_temp_ppt_", N_advance,".rds"))
