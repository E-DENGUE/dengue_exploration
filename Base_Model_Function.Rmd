...
#save(base_model_final_2, file = "base_model_final_2.RData")
...
#advance_missing_base_model <- advance_missing_func(8)
#save(advance_missing_base_model, file = "advance_missing_base_model.RData")
...
# posterior_quant <- as.data.frame(fitted)
 #posterior_quant_df <- lapply(X = matrix(results$samples$fitted, results$samples$Y),FUN = samples_to_quant)
 #posterior_quant_df <- quantile(results$samples$fitted, probs=c(0.025, 0.975,seq(0.01, 0.99, by=0.01)))
 #posterior_quant_df <- lapply(results$samples$fitted, quantile, probs=c(0.025, 0.975,seq(0.01, 0.99, by=0.01)))
  #posterior_quant <- lapply(results$samples$fitted, quantile, probs=c(0.025, 0.975))
  #posterior_fitted <- results$fitted
 ...
 #saveRDS(posterior_quant, paste0("posterior_quant_", N_advance,".rds"))
 #saveRDS(posterior_fitted, paste0("posterior_fitted_", N_advance,".rds"))
  return(0)
}
...
## Model Checking
```{r}
traceplot(mod_aug_2007$samples$fitted[,1:20], smooth = FALSE,
       col = 1:6, type = "l", xlab = "Iterations", ylab = "")

convergence_plot <- mod_check$samples$fitted
...
geweke.plot(mod_aug_2007$samples$fitted[, 1:20], frac1 = 0.1, frac2 = 0.5, nbins = 20,
...
summary(fit$samples$fitted)

mod_aug_2007$summary.results
