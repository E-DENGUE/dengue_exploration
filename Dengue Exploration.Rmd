...
base_model_final %>%
  mutate(m_DHF_cases = if_else(m_DHF_cases == 0, 0.1, m_DHF_cases), 
         m_DHF_cases = if_else(is.na(m_DHF_cases), 0.1, m_DHF_cases), 
         log_inc = log(m_DHF_cases/pop*100000)) %>%
ggplot()+
  geom_sf(aes(geometry = geometry, fill = log_inc, na.rm = T), lwd = 0, size = 0.25)+
  facet_wrap(~factor(month))+
  viridis::scale_fill_viridis(option = "magma", 
                              direction = -1, 
                              name = "DHF Incidence (log per 100,000)", 
                              oob = scales::squish)+
   theme(axis.text.x = element_blank(),
        axis.text.y = element_blank(),
        axis.ticks = element_blank(),
        panel.background = element_rect(fill = "white", color = NA)) +
  labs(title = "Average Monthly DHF Incidence 2001 - 2018", subtitle = "Grouped by month")

base_model_final_2 <- base_model_final %>%
  ungroup() %>%
  group_by(VARNAME_2) %>%
  mutate(total_cases = sum(total_DHF_cases, na.rm = T), 
            avg_pop = mean(pop, na.rm = T),
         inc = total_cases/avg_pop*100000,
            log_inc = log(total_cases/avg_pop*100000), 
         inc = if_else(is.na(inc), 0, inc),
         log_inc = if_else(is.na(log_inc), 0, log_inc)) %>%
  distinct(geometry, .keep_all = T) 

ggplot(data = base_model_final_2)+
  geom_sf(aes(geometry = geometry, fill = log_inc, na.rm = T), lwd = 0, size = 0.25)+
   viridis::scale_fill_viridis(option = "magma", 
                              direction = -1, 
                              name = "DHF Incidence (log per 100,000)", 
    ...
    theme(axis.text.x = element_blank(),
        ...
        axis.ticks = element_blank(),
        panel.background = element_rect(fill = "white", color = NA))+
  labs(title = "Average DHF Incidence 2001 - 2018")
 
###### heat maps 
base_model_final_3 <- base_model_final %>%
  ungroup() %>%
  group_by(VARNAME_2, year) %>%
  summarize(tot_cases = sum(m_DHF_cases, na.rm = T)) %>% 
  mutate(tot_cases = if_else(is.na(tot_cases), 0, tot_cases)) %>%
  ungroup() %>%
  group_by(VARNAME_2) %>%
  mutate(scaled_cases = tot_cases/max(tot_cases))
  


# by each district
ggplot(base_model_final_3, aes(year, VARNAME_2, fill = scaled_cases))+
  geom_tile()+ 
  scale_fill_distiller(name = "DHF Cases", 
                       palette = "Blues", 
                       direction = 1,
