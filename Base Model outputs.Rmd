...
# model output
mod_aug_2007_2 <- as.data.frame(mod_aug_2007$fitted.values)
...
# model output
post_quant_1 <- readRDS("Data/posterior_quant_1 (2).rds")
...
## Model 10
```{r}
base_model_10 <- base_model_final_2 %>%
  mutate(year = unfactor(year), 
         date = as.Date(paste(year, month, '01', sep='-'))) %>%
  filter(date <= (as.Date('2006-12-01' ) %m+% months(10))) %>%
  mutate(t = row_number())

# model output
post_quant_10 <- readRDS("Data/posterior_quant_10.rds")

# 2.5% CI
post_quant_10_2.5 <- as.data.frame(post_quant_10)[1,]
post_quant_10_2.5 <- post_quant_10_2.5  %>%
  pivot_longer(cols = starts_with("var")) %>%
  mutate(t = row_number())

# 97.5% CI
post_quant_10_97.5 <- as.data.frame(post_quant_10)[2,]
post_quant_10_97.5 <- post_quant_10_97.5  %>%
  pivot_longer(cols = starts_with("var")) %>%
  mutate(t = row_number())

# join to base model
model_check <- left_join(base_model_10, post_quant_10_2.5, by = c("t" = "t"))
model_check_10 <- left_join(model_check, post_quant_10_97.5, by = c("t" = "t"))

# calculate incidence
model_check_10 <- model_check_10 %>%
  group_by(VARNAME_2, year, month) %>%
  mutate(monthly_inc = (m_DHF_cases/pop)*100,000, 
         fitted_inc_2.5 = (value.x/pop)*100,000, 
         fitted_inc_97.5 = (value.y/pop)*100,000)
...
model_check_10 %>%
  ...
  geom_errorbar(aes(date, ymin = fitted_inc_2.5, ymax = fitted_inc_97.5), color = "red")+
  theme_classic()+
  labs(title = "Vinh Hung, 10", subtitle = "Incidence per 100,000")
...
model_check_10 %>%
  ...
  geom_errorbar(aes(date, ymin = value.x, ymax = value.y), color = "red")+
  theme_classic()+
  labs(title = "Cai Lay District, 10")
...
model_check_10 %>%
  ...
  geom_errorbar(aes(date, ymin = fitted_inc_2.5, ymax = fitted_inc_97.5), color = "red")+
  theme_classic()+
  labs(title = "Cai Lay District, 10", subtitle = "Incidence per 100,000")
...
model_check_10 %>%
  ...
  geom_errorbar(aes(date, ymin = value.x, ymax = value.y), color = "red")+
  theme_classic()+
  labs(title = "Phu Tan An Giang, 10")
...
model_check_10 %>%
  ...
  geom_errorbar(aes(date, ymin = fitted_inc_2.5, ymax = fitted_inc_97.5), color = "red")+
  theme_classic()+
  labs(title = "Phu Tan An Giang, 10", subtitle = "Incidence per 100,000")
...
# model output
post_quant_20 <- readRDS("Data/posterior_quant_20.rds")
...
# model output
post_quant_40 <- readRDS("Data/posterior_quant_40.rds")
...
# model output
post_quant_60 <- readRDS("Data/posterior_quant_60.rds")
...
# model output
post_quant_80 <- readRDS("Data/posterior_quant_80.rds")
...
# model output
post_quant_100 <- readRDS("Data/posterior_quant_100.rds")
...
# model output
post_quant_120 <- readRDS("Data/posterior_quant_120.rds")
...
# model output
post_quant_144 <- readRDS("Data/posterior_quant_144.rds")
...
saveRDS(summary(mod_aug_2007), "mod_sum.rds")
```
