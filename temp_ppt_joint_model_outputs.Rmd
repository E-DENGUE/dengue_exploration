---
title: "Temp + Precip Model Outputs"
...
```{r, include=FALSE}
## Climate Model - Observed Cases
climate_model <- readRDS("Data/climate/climate_model.rds")

climate_model_2 <- climate_model %>%
...
display.func <- function(N_advance){
  # observed cases
  climate_model_for_model <- climate_model_2 %>%
  mutate(year = unfactor(year), 
         date = as.Date(paste(year, month, '01', sep='-'))) %>%
  ...
  # model output
  climate_fit <- readRDS(paste0("temp_ppt_joint_model_outputs/post_fit_joint_temp_ppt_", N_advance, ".rds"))
  
  climate_df <- as.data.frame(climate_fit)
  climate_df <- climate_df  %>%
    mutate(t = row_number())
  
  # join to base model
  model_check <- left_join(climate_model_for_model, climate_df, by = c("t" = "t"))
  ...
  model_check <- model_check %>%
    ...
    mutate(monthly_inc = (m_DHF_cases/pop)*100,000,
         ...
         fitted_inc = (climate_fit/pop)*100,000, 
...
#    labs(title = paste0("Observed vs. Fitted Monthly Incidence", N_advance))
  ...
  filter(date == (as.Date('2006-12-01' ) %m+% months(N_advance))) %>%
  mutate(mae = abs(monthly_inc - fitted_inc))
  saveRDS(mae_df, paste0("Data_4/temp_ppt_joint_MAE_df", N_advance, ".rds"))
...
mae_merged_df <- do.call(rbind, 
                         lapply(list.files('/Users/zoe/Documents/Yale EMD/Rotation 1/Dengue - Vietnam/dengue_exploration/Data_4', 
                        recursive = TRUE, full.names = TRUE), readRDS))
saveRDS(mae_merged_df, "temp_ppt_joint_model_mae.rds")
...
MAE_full <- readRDS("temp_ppt_joint_model_mae.rds")
